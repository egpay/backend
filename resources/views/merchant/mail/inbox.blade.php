...
<div class="modal fade text-xs-left" id="send-email-modal"  role="dialog" aria-labelledby="myModalLabel33" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            {!! Form::open(['id'=>'send-email-form','method'=>'POST','route'=>'panel.merchant.mail.store','onsubmit'=>'return false;']) !!}
...
@section('content')
        <div class="card mt-0">
            <div class="card-block">
                <div class="app-content container center-layout mt-0 email-application">
                            ...
                            <div class="sidebar-content email-app-sidebar">
                                <div class="email-app-menu col-md-5 card hidden-md-down">
                                    ...
                                    <div class="list-group list-group-messages">
                                        ...
                                        <a href="{{route('panel.merchant.mail.index',['type'=>'inbox'])}}" @if($type == 'inbox') class="list-group-item active no-border" @else class="list-group-item list-group-item-action no-border" @endif>
                                            <i class="ft-inbox mr-1"></i> {{__('Inbox')}}
                                            @if($inboxCount)
                                                <span class="tag tag-default tag-pill float-xs-right">{{$inboxCount}}</span>
                                            @endif
                                        </a>
                                        <a href="{{route('panel.merchant.mail.index',['type'=>'sent'])}}" @if($type == 'sent') class="list-group-item active no-border" @else class="list-group-item list-group-item-action no-border" @endif><i class="fa fa-paper-plane-o mr-1"></i> Sent</a>
                                        <a href="{{route('panel.merchant.mail.index',['type'=>'star'])}}" @if($type == 'star') class="list-group-item active no-border" @else class="list-group-item list-group-item-action no-border" @endif><i class="ft-star mr-1"></i> Starred </a>
                                    </div>

                                </div>
                                <div class="email-app-list content-wraper col-md-7 card p-0">
                                    <div class="email-app-list">
                                        <div class="card-block chat-fixed-search" style="padding: 1rem !important;">
                                            {!! Form::open(['route'=>'panel.merchant.mail.index','method'=>'GET'])  !!}
...
@section('footer')
    ...
    <script type="text/javascript">
        ...
        function readMail($id){
            ...
            $actionURL = '{{url()->route('panel.merchant.mail.index')}}';
