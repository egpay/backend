        ...
        @foreach($result['data']->settlement as $key => $value)
                <div class="col-md-12">
                    ...
                    <table class="table table-bordered table-striped">
                        <thead>
                        <tr>
                            <th colspan="7" style="    background-color: aliceblue;">
                                <h3 style="text-align: center;">
                                    {!! adminDefineUser($value['model_type'],$value['model_id'],__('#ID').':'.$value['model_id'].' - '.$value['model']['name_'.$systemLang]) !!}
                                </h3>
                            </th>
                        </tr>
                        <tr>
                        ...
                        @foreach($value['success'] as $keySuccess => $valueSuccess)
                            <tr>
                                <th>
                                    ...
                                    <a href="{{route('payment.invoice.show',$valueSuccess['invoice_id'])}}">{{$valueSuccess['invoice_id']}}</a>
                                </th>

                                <th>
                                    {{--{{route('system.invoice.show',[$valueSuccess['invoice_id']])}}--}}
                                    <a href="{{route('payment.services.show',$valueSuccess['service_id'])}}">{{$valueSuccess['service_name']}}</a>
                                </th>
                        ...
                        <tr>
                            ...
                            <td colspan="5">{{amount($value['system_commission'])}}</td>
                        </tr>

                        <tr>
                            <td colspan="2">{{__('Merchant Commission')}}</td>
                            <td colspan="5">{{amount($value['merchant_commission'])}}</td>
                        </tr>
                    ...
                    @if(!empty($value['error']))
                        <table class="table table-bordered table-striped">
                            <thead>
                                <tr>
                                    <th colspan="7" style="color:#FFF;background-color: red;">
                                        <h3 style="text-align: center;">
                                            {{__('Errors')}}
                                        </h3>
                                    </th>
