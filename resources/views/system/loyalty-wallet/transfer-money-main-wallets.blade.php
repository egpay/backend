                                ...
                                <div class="col-sm-12">
                                    <div class="card">
                                        <div class="alert @if(!is_object(session('transactionStatus'))) alert-danger @else alert-success @endif">
                                            ...
                                            @else
                                                {!! __('The money was successfully transferred (Transaction ID:').' <a href="'.route('system.wallet.transactions.show',['id'=>session('transactionStatus')->id]).'">'.session('transactionStatus')->id.'</a>)' !!}
                                            @endif
                                        </div>
                                    </div>
                                </div>
                ...
                var html = "<label for=\"transfer_type\">{{__('Wallet')}}:</label>\n" +
                    "<select class=\"form-control\" id=\"main_wallet_id\" name=\"main_wallet_id\">\n" +
                        @foreach($walletOUT as $key => $value)
                            "<option value=\"{{$value->id}}\">{{$value->name}} ({{amount($value->wallet->balance,true)}})</option>\n" +
                        @endforeach
                            "</select>";
                $('#main_wallet_content').html(html);
