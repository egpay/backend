...
class Commission
{
    ...
    private static $commissionData = [], $dataAfterSettlement = [];
    ...
    private static function getCommissionData($ID){
        if (isset(self::$commissionData[$ID])) {
            return self::$commissionData[$ID];
        }

        self::$commissionData[$ID] = CommissionList::where('id', $ID)->first();
        return self::$commissionData[$ID];
    }

    public static function calculateCommission($amount, int $commissionListID){
        $commissionData = self::getCommissionData($commissionListID);
        if (!$commissionData) {
    ...
    public static function paymentSettlement($formDateTime, $toDateTime, $modelPath = 'App\Models\Merchant', $modelID = null)
    {
        ...
        foreach ($invoicesByMerchants as $keyMerchant => $merchant) {
            ...
            whereBetween($getInvoicesEloquent, 'payment_invoice.updated_at', $formDateTime, $toDateTime);
            ...
            foreach ($getInvoices as $key => $value) {
                $calculateCommission = self::calculateCommission($value->total_amount, $value->commission_list_id);
