...
class StaffInfoApiController extends MerchantApiController
{
    ...
    public function changePassword(){
        $userObj = Auth::user();
        $RequestData = array_filter($this->headerdata(['password','password_confirmation','current_password','revokeALl']));
        $validator = Validator::make($RequestData, [
            'current_password'  => 'required|PasswordCheck:'.$userObj->password,
            'password'          => 'required|confirmed|min:6',
            'revokeALl'         => 'required|in:0,1',
        ]);
        if($validator->errors()->any()){
            return $this->ValidationError($validator,__('Validation Error'));
        }
        ...
        try {
            ...
            if($pwd && ($RequestData['revokeALl']==1)){
                foreach ($userObj->tokens as $onetoken) {
                    $onetoken->revoke();
                }
            }
